...
*Append each echo command to the new script file (gatk_combine_gvcf_run.sh) using `>>`. This will allow you to save the text output and rapidly regenerate the combined GVCF file. (Warning: extra spaces and tabs will break the script!)
...
#### 5. Now that you have created your `gatk_combined_gvcf_create.txt.sh` script, run it using `bash` to create a script with input text files and the `gatk` CombineGVCFs tool commands.
```
...
```
*Copy of `gatk_combined_gvcf_create.txt.sh` script output text contained in the generated `gatk_combine_gvcf_run.sh` script.
```
...
#### 12. Filter using the 'gatk' recommended filtering parameters to compare filtering. `gatk` filters help with strand bias, filtering for quality based upon depth, etc.
